...
class ACNet(object):
    ...
    def __init__(self, scope, opt_a=None, opt_c=None, global_net=None):
        ...
        if scope == 'global_net':  # get global network
            ...
            with tf.variable_scope(scope):  #本地任务是那个worker
                self.s = tf.placeholder(tf.float32, [None, N_S], 'S')   #表示状态
                self.a_his = tf.placeholder(tf.int32, [None, ], 'A')    #表示历史动作
                self.v_target = tf.placeholder(tf.float32, [None, 1], 'Vtarget')

                self.a_prob, self.v, self.a_params, self.c_params = self._build_net(scope)   #构建本地网络

                td = tf.subtract(self.v_target, self.v, name='TD_error')  #获取td_error
                ...
                with tf.name_scope('a_loss'):
                    log_prob = tf.reduce_sum(
                        ...
                        axis=1, keep_dims=True)  #给出概率密度函数
                    exp_v = log_prob * tf.stop_gradient(td)  #停止点的期望值
                    entropy = -tf.reduce_sum(self.a_prob * tf.log(self.a_prob + 1e-5),
                                             axis=1, keep_dims=True)  # encourage exploration  计算熵值
                    self.exp_v = ENTROPY_BETA * entropy + exp_v   #联合计算出期望值
                    self.a_loss = tf.reduce_mean(-self.exp_v)   #计算损失
    ...
    def _build_net(self, scope):
        w_init = tf.random_normal_initializer(0., .1)  #用来初始化权重
        ...
        a_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope + '/actor')  #actor参数
        c_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope + '/critic') #critic参数
        return a_prob, v, a_params, c_params

    def choose_action(self, s):  # run by a local
        prob_weights = self.sess.run(self.a_prob, feed_dict={self.s: s[np.newaxis, :]})   #获取权重
        action = np.random.choice(range(prob_weights.shape[1]),
                                  p=prob_weights.ravel())  # select action w.r.t the actions prob 选择动作
        return action
...
def work(job_name, task_index, global_ep, lock, r_queue, global_running_r):
    ...
    if job_name == 'ps':
        ...
        t1 = time.time()
        env = gym.make('CartPole-v0').unwrapped   #游戏环境
        ...
        local_net = ACNet('local_ac%d' % task_index, opt_a, opt_c, global_net)  #构建本地网络
        # set training steps
        ...
        with tf.train.MonitoredTrainingSession(master=server.target,
            ...
            while (not sess.should_stop()) and (global_ep.value < 1000):
                ...
                while True:
                    # if task_index:
                    #     env.render()
                    a = local_net.choose_action(s)  #本地网络选择动作
                    s_, r, done, info = env.step(a)  #和环境交互
                    if done: r = -5.
                    ep_r += r
                    buffer_s.append(s)  #状态缓存
                    buffer_a.append(a)  #动作缓存
                    buffer_r.append(r)  #奖励缓存
                    ...
                    if done:
                        if r_queue.empty():  # record running episode reward
                            ...
                            global_running_r.value = .99 * global_running_r.value + 0.01 * ep_r   #全局奖励
